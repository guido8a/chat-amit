  ...
  return <>
    <Box sx={{...accordeonBox.container, pt:'2rem', height: '100%', backgroundImage: 'url(src/style/MySvgIcons)'}}>
      <AppBar position='fixed'
        ...
        <SectionTitle title={'Elaborar Acto Administrativo'} />
      </AppBar>
      <Grid container spacing={1} sx={accordeonBox.container2}>
        ...
        <Grid item xs={12} sx={dialog.titleContainer}>
          <MySubtitle subtitle={'Acto Administrativo'} />
        </Grid>

        <Grid item xs={12}>
          <MyUpload id={'actoAdministrativoRespaldo'}
                    label={'Respaldo *'}
        ...
        <Grid item xs={12} style={{padding:'0 0 0 24px'}}>
          <MyAreaTextField id={'actoAdministrativo'}
                           label={'Contenido *'}
                           formValues={formValues}
            ...
            <Grid item xs={12} sx={{p:0, m:0, position: 'sticky',}}>
              <Stack direction={'row'} justifyContent="space-between" alignItems="center" sx={{p:'0 4rem 0 4rem'}}>
                ...
                <MyButtonBacan label={'generar PDF'}
                               onClick={() => {
                                 API.fetchGenerarPDF(instanciaProceso?.solicitud?.id, 'acto_adminis6trativo','resolucion',formValues.actoAdministrativo)
                                   .then(result => {
                ...
                <MyButtonBacan label={'bajar PDF'}
                               onClick={() => {
                                 const url = `${GLOBALS.mainUrl}/documentos/descargar?filename=vuv-${instanciaProceso?.solicitud?.id}/acto_adminis6trativo/${formValues.pdf}`
                                 fetch(url)
                                   .then((res) => { return res.blob(); })
                                   .then((data) => {
                                     const dataPdf = new Blob([data], { type: 'application/pdf' })
                ...
                <MySendButton onSend={ () => {
                              ...
                              disabled={
                                formValues.pdf === '' ||
                                formValues.actoAdministrativo === '' ||
                                formValues.actoAdministrativoRespaldo === '' ||
                                counter > 0
                              } />
              </Stack>
            </Grid>
