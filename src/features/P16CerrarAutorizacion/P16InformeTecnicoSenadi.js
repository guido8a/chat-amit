...
export default ({instanciaTarea, instanciaProceso, solicitud, perfilUsuario, solicitudesAprobadas}) => {
  ...
  const section = 'EmitirInformeTecnicoSenadi'
  ...
  return <>
    <Box sx={{...accordeonBox.container, pt:'2rem', height: '100%', backgroundImage: 'url(src/style/MySvgIcons)'}}>
      <AppBar position='fixed'
        ...
        <SectionTitle title={'Emitir Informe Técnico'} />
      </AppBar>
      <Grid container spacing={1} sx={accordeonBox.container2}>
        ...
        <Grid item xs={12} sx={dialog.titleContainer}>
          <MySubtitle subtitle={'Respaldo'} />
        </Grid>
        <Grid item xs={12}>
          <Stack direction={'row'} justifyContent="flex-end" alignItems="center" sx={{p:'0 1rem 0 4rem'}}>
        ...
        <Grid item xs={12} style={{padding:'0 0 0 24px'}}>
          <Stack direction={'row'} justifyContent="flex-end" alignItems="center" sx={{p:'0 1rem 0 4rem'}}>
            <MyButtonBacan label={'ver PDF'}
                           onClick={() => {
                             const url = `${GLOBALS.mainUrl}/documentos/descargar?filename=vuv-${instanciaProceso?.solicitud?.id}/IT-Senadi/${payload.InformeTecnicoSenadi.pdf}`
                             fetch(url)
                               .then((res) => { return res.blob(); })
                               .then((data) => {
                                 const dataPdf = new Blob([data], { type: 'application/pdf' })
            ...
            <Grid item xs={12} sx={{p:0, m:0, position: 'sticky',}}>
              <Stack direction={'row'} justifyContent="space-between" alignItems="center" sx={{p:'0 4rem 0 4rem'}}>
                ...
                <MyButtonBacan label={'generar PDF'}
                               onClick={() => {
                                 API.fetchGenerarPDF(instanciaProceso?.solicitud?.id, 'DT-Senadi','informe_tecnico',formValues.informe)
                                   .then(result => {
                ...
                <MyButtonBacan label={'bajar PDF'}
                               onClick={() => {
                                 const url = `${GLOBALS.mainUrl}/documentos/descargar?filename=vuv-${instanciaProceso?.solicitud?.id}/DT-Senadi/${formValues.pdf}`
                                 fetch(url)
                                   .then((res) => { return res.blob(); })
                                   .then((data) => {
                                     const dataPdf = new Blob([data], { type: 'application/pdf' })
